...
// Prop olarak veri alan bir bileşeni test ediyorsak bileşenin aldığı propları test ortamında da göndermemiz gerekir
test("Miktar,başlık ve fotoğraf gelen propa göre ekrana basılır", () => {
    ...
    // span içereği 3 mü kontrol et
    expect(amount.textContent).toBe("3");

    // chocolate yazısı ekrana geldimi kontrol et
    // getBy elementi bulunmazsa hata fırlatır bu yüzden sadece "x" yazı içeriğine sahip element ekranda mı ?
    // kontrolü yapmak istiyorsak getByText ile elementi çağırmak yeterlidir daha sonrasında expext kullanmaya bile gerek kalmaz
...
test("Butonlara tıklanınca fonksiyonlar doğru parametre ile çalışır", async () => {
    ...
    // prop olarak gönderilen fonksiyonu test edeceksek jest aracılığı ile test (mock) ediliebilir fonksiyonlar oluştur
